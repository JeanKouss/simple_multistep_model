...
PERIOD_TOKEN_RE = re.compile(r"^\d{4}-(\d{2}|W\d{2})$")


def extract_period_token(time_period: str) -> str:
    """Extract MM or Wnn token from YYYY-MM / YYYY-Wnn strings."""
    value = str(time_period).strip()
    match = PERIOD_TOKEN_RE.match(value)
    if not match:
        raise ValueError(
            "time_period must match YYYY-MM or YYYY-Wnn; got "
            f"{time_period!r}"
        )
...
class ResidualBootstrapModel:
    ...
    def fit(
        ...
        for residual, (location, period_token) in zip(self._residuals, residual_context, strict=True):
            loc = str(location)
        ...
        self._residuals_by_location_period = {
            key: np.asarray(values, dtype=float) for key, values in by_location_period.items()
        }
